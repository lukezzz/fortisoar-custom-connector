...
class HuaweiOSConnect:
        ...
        if self._shell_channel is not None:
            ...
            for cmd in cmd_list:
                try:
                    ...
                    logger.info("Exception Occurred {0}".format(error))
                    logger.info("Exception Occurred in command {0}".format(cmd))
    ...
    def get_config_info(self, params):
        """
        Function that executes the display current-configuration command
        """
