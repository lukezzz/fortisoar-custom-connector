...
HEALTH_CHECK_XPATH = "/config/devices/entry/vsys/entry[@name='{vsys_name}']/rulebase/security"

URL_XPATH = "/config/devices/entry[@name='localhost.localdomain']/vsys/entry[@name='{vsys_name}']/profiles/" \
            "custom-url-category/entry[@name='{url_profile_name}']"

APPLICATION_XPATH = "/config/devices/entry[@name='localhost.localdomain']/vsys/entry" \
                    "[@name='{vsys_name}']/application-group/entry[@name='{app_group_name}']"

IP_XPATH_GROUP = "/config/devices/entry[@name='localhost.localdomain']/vsys/entry" \
                 "[@name='{vsys_name}']/address-group/entry[@name='{address_group}']"

IP_ADDRESS_XPATH = "/config/devices/entry[@name='localhost.localdomain']/vsys/entry" \
